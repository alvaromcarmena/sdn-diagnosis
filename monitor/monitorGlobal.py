...
netNodes = ['S1_rx_packets_level', 'S1_tx_packets_level', 'S1_lost_packets_level', 'S1_auxPort_up', 'S1_outPort_up', 'S1_inPort_up', 'S1_sw_up', 'S2_rx_packets_level', 'S2_tx_packets_level', 'S2_lost_packets_level', 'S2_auxPort_up', 'S2_outPort_up', 'S2_inPort_up', 'S2_sw_up', 'S3_rx_packets_level', 'S3_tx_packets_level', 'S3_lost_packets_level', 'S3_auxPort_up', 'S3_outPort_up', 'S3_inPort_up', 'S3_sw_up', 'S4_rx_packets_level', 'S4_tx_packets_level', 'S4_lost_packets_level', 'S4_auxPort_up', 'S4_outPort_up', 'S4_inPort_up', 'S4_sw_up', 'S5_rx_packets_level', 'S5_tx_packets_level', 'S5_lost_packets_level', 'S5_auxPort_up', 'S5_outPort_up', 'S5_inPort_up', 'S5_sw_up', 'S6_rx_packets_level', 'S6_tx_packets_level', 'S6_lost_packets_level', 'S6_auxPort_up', 'S6_outPort_up', 'S6_inPort_up', 'S6_sw_up', 'S7_rx_packets_level', 'S7_tx_packets_level', 'S7_lost_packets_level', 'S7_auxPort_up', 'S7_outPort_up', 'S7_inPort_up', 'S7_sw_up', 'S8_rx_packets_level', 'S8_tx_packets_level', 'S8_lost_packets_level', 'S8_auxPort_up', 'S8_outPort_up', 'S8_inPort_up', 'S8_sw_up', 'root_cause_failure', 'forecast', 'probability']

def _create_csv():
    csvsalida = open('netstats.csv', 'w', newline='')
    salida = csv.writer(csvsalida)
    salida.writerow(netNodes)
    del salida
    csvsalida.close()
    logger.info('csv created')

def _update_csv(variables):
    csvsalida = open('netstats.csv', 'a', newline='')
    salida = csv.writer(csvsalida)
    salida.writerow(variables)
    del salida
    csvsalida.close()
...
#se establece un periodo temporal de 14 segundos, y se envían las variables procesadas al módulo bayesiano
    while True:
        try:
                ...
                rcf = _read_rcf()
                #se cogen las variables de todos los switches y se actualiza el csv una vez con todas las variables recogidas                
                for dpid in networkSwitches:
                   txLevel, rxLevel, lostLevel, auxPortState, outPortState, inPortState, swState = _get_variables(dpid)       
                  
                   variables.append(rxLevel)
                   variables.append(txLevel)
                   variables.append(lostLevel)
                   variables.append(str(auxPortState))
                   variables.append(str(outPortState))
                   variables.append(str(inPortState))
                   variables.append(str(swState))
                   #print(dpid, "Tx packets:", txLevel,"Rx_packets:", rxLevel,"Lost_packets:", lostLevel, "inPortUp:", inPortState, "outPortUp:", outPortState, "auxPortUp:", auxPortState, "swUp:", swState, "rcf:", rcf )
                
                forecast, prob = _get_forecast(variables)
                variables.append(rcf)
                variables.append(forecast)
                variables.append(prob)
                #print(rcf, forecast, prob)
            
                _update_csv(variables)
...
#se procesan las distintas variables haciéndolas discretas y se devuelven 
   ...
   auxPortState = _port_state(dpid, auxPort)
   outPortState = _port_state(dpid, outPort)
   inPortState = _port_state(dpid, inPort)
...
def _calculate_packets_hostSwitch(dpid):
   ...
   txpackets = stats[0]['switches'][dpid]['port_stats'][outPort]['tx_packets'] + stats[0]['switches'][dpid]['port_stats'][auxPort]['tx_packets'] - stats_before[associations[0][dpid]]['sw_stats']['tx_packets']
   rxpackets = stats[0]['switches'][dpid]['port_stats'][inPort]['rx_packets'] - stats_before[associations[0][dpid]]['sw_stats']['rx_packets']
   #se considera que no se pueden perdr paquetes en los switches de host, ya que pox no permite recoger estadísticas de estos
   okpackets = txpackets
   lostpackets = txpackets - okpackets
   #print(dpid, "Tx packets:", txpackets, "Rx packets:", rxpackets,"OK packets:", okpackets, "Lost packets:", lostpackets)
...
#se añaden las evidencias recibidas en el parámetro y se devuelve el valor del nodo root cause failure de más probabilidad   
    for i in range(len(netNodes)-3): 
        net.setEvidence(netNodes[i], variables[i]);
...
if __name__ == '__main__':
    ...
    net.readFile("/home/mininet/sdn-diagnosis/monitor/bayesianNetworkGlobal.xdsl")
    ...
    #se añaden 8 diccionarios a la lista stats_before uno para cada switch    
    x = 0
    while x <= 8:
